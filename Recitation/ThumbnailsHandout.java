...
public class ThumbnailsHandout extends DrawingGUI {
	...
	private static BufferedImage thumbnailify(BufferedImage image) {
		...
		// TODO: Your code here
		int imageHeight = image.getHeight(); // w 
		int imageWidth = image.getWidth(); // h 
		
		//nested for loop looping through every pixel
		System.out.println("IMAGE HEIGHT: " + imageHeight);
		System.out.println("IMAGE WIDTH: " + imageWidth);
		for(int y = 0; y < thumbHeight; y++) { 
			for(int x = 0; x < thumbWidth; x++) {
				int i = x * (imageHeight / thumbHeight);
				int j = y * (imageWidth / thumbWidth); 
				result.setRGB(x, y, image.getRGB(i, j));
			}
		} 
		return result; 
	}
	...
	public void draw(Graphics g) {
		...
		for (int i=0; i < trows; i++) {
			for (int j=0; j < tcols; j++) {
				g.drawImage(thumbs.get(i*tcols+j), i*thumbWidth, j*thumbHeight, null);
			}
		}
	}
