 ...
 * 
 * @author Jeremy Hadfield, Spring 2019, updated for PS2
 */
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			// TODO: YOUR CODE HERE
			Dot new_point = new Dot(x, y); 
			if(tree == null) { 		// if tree doesn't already exist, create a new tree
				tree = new PointQuadtree(new_point, 0, 0, width, height); 
			} else { 				// if tree already exits, add the point to the tree
	...
	/** 
	 * Method to test tree 2
	 */
	private void test2() { 
		found = null; 
		// start at center of the screen
		tree = new PointQuadtree<Dot>(new Dot((int) width/2, (int) height/2), 0, 0, width, height); // A
		// insert new dots into the tree at specified locations
		tree.insert(new Dot(300, 500)); // B
		tree.insert(new Dot(100, 130)); // C
		int bad = 0;  // no errors initially; if bad > 0, error has occurred
		bad += testFind(width/2, height/2, 5, 3, 3, 1); // only gets A; rect for A [B] [C]; circle for A; finds A
		bad += testFind(300, 500, 5, 3, 2, 1); // only gets B; rect for A [B] [C]; circle for A,B; finds B
		bad += testFind(0, 0, 900, 3, 3, 3); // gets them all; rect for all; circle for all; finds all
		if(bad == 0) { 
			System.out.println("yep, we are a GO for test 2!"); 
		} else { 
			System.out.println("nope, we are a NO GO for test 2!"); 
		}
	}
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		// draw the point 
		g.fillOval((int)treeX-dotRadius, (int)treeY-dotRadius, mouseRadius, mouseRadius); 
